...
dotsync intentionally excludes dynamic local Serena MCP URLs from Codex config sync. Serena ports are per-project runtime state and are injected by the local launcher when Codex starts, so a copied `127.0.0.1:<port>` URL is treated as machine-local state rather than user-authored config.

For local Serena MCP sharing, use a zsh agent shim instead of installing `codex` or `claude` wrapper files. The shim defines `codex()` and `claude()` in `~/.zshrc`, preserves the existing session/memory preflight cleanup for interactive no-argument launches, then calls the installed launcher at `$(brew --prefix dotsync)/libexec/tools/serena_agent_launcher.py`. When an interactive agent exits, the launcher prints a Serena shutdown row showing the session count before exit, the closed lease count, the remaining lease count, and whether the scoped server was kept or stopped. Generate the snippet with `python3 "$(brew --prefix dotsync)/libexec/tools/serena_zsh_shim.py"` and source `~/.zshrc` after writing it.
...
dotsync 는 Codex 설정을 sync 할 때 동적 로컬 Serena MCP URL 을 의도적으로 제외한다. Serena 포트는 프로젝트별 runtime state 이고 Codex 시작 시 로컬 launcher 가 주입하므로, 복사된 `127.0.0.1:<port>` URL 은 사용자가 작성한 설정이 아니라 머신 로컬 상태로 취급한다.

로컬 Serena MCP 공유는 `codex`/`claude` wrapper 파일 설치 대신 zsh agent shim을 사용한다. shim은 `~/.zshrc`에 `codex()`와 `claude()` 함수를 정의하고, 인자가 없는 대화형 실행에서만 기존 session/memory preflight cleanup을 유지한 뒤 설치된 launcher인 `$(brew --prefix dotsync)/libexec/tools/serena_agent_launcher.py`를 호출한다. 대화형 agent가 종료되면 launcher는 종료 전 session 수, 닫은 lease 수, 남은 lease 수, scoped server 유지/종료 여부를 Serena shutdown row로 출력한다. snippet은 `python3 "$(brew --prefix dotsync)/libexec/tools/serena_zsh_shim.py"`로 생성하고, 작성 후 `~/.zshrc`를 다시 source한다.
