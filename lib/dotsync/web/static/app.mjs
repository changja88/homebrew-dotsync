...
const MANAGER_SYNC_LISTENER_PROBE_EVENT = "dotsync:manager-sync-listener-probe";
...
if (launch) {
  ...
  function postManagerSyncSignal(action) {
    if (
    ...
    handler.postMessage(Object.freeze({ action }));
    return true;
  }
  ...
  window.addEventListener(MANAGER_SYNC_HANDOFF_EVENT, async (event) => {
    if (await receiveManagerSyncHandoff(event)) {
      postManagerSyncSignal("manager_sync_handoff_received");
    }
  });
  window.addEventListener(MANAGER_SYNC_LISTENER_PROBE_EVENT, () => {
    postManagerSyncSignal("manager_sync_listener_ready");
  });
  postManagerSyncSignal("manager_sync_listener_ready");
