...
if (launch) {
  ...
  function nativeHandler() {
    const handler = window.webkit?.messageHandlers?.dotsync;
    return handler && typeof handler.postMessage === "function" ? handler : null;
  }

  function postNative(action, destination = null) {
    const handler = nativeHandler();
    if (!handler) {
      return false;
    }
    if (action === "open_manager" && ["overview", "accounts", "sync", "settings"].includes(destination)) {
      handler.postMessage({ action: "open_manager", destination });
      return true;
    }
    if (action === "refresh_summary") {
      handler.postMessage({ action: "refresh_summary" });
      return true;
    }
    if (action === "quit_app") {
      handler.postMessage({ action: "quit_app" });
      return true;
    }
    return false;
  }
  ...
  async function routePreview(direction) {
    ...
    preview = null;
    if (state.surface === "popover" && postNative("open_manager", "sync")) {
      return;
    }
    await navigate("sync");
  }
  ...
  function pollJob(jobId, accountId = null, delay = 500) {
    const poll = async () => {
      if (stopped) {
        return;
      }
      if (hiddenSince !== null && performance.now() - hiddenSince >= 30_000) {
        pollingTimers.delete(jobId);
        return;
      }
      try {
        const payload = await api.job(jobId);
        state = reduce(state, { type: "JOB_UPDATED", job: payload.job });
        render();
      ...
      const timer = window.setTimeout(() => pollJob(jobId, accountId, nextDelay), delay);
  ...
  async function createSyncPreview() {
    try {
      const payload = await api.syncPreview(activeDirection, [...selectedApps]);
  ...
  async function executeSync() {
    ...
    const isApply = activeDirection === "apply";
    const result = await confirmationDialog({
  ...
  async function handleAction(target) {
    ...
    if (action === "navigate") {
      ...
      activeDirection = target.dataset.direction;
      preview = null;
      render();
