...
export function readLaunchContext(location, history) {
  ...
  const allowed = new Set(["token", "destination"]);
  ...
  return Object.freeze({ token, destination });
}
...
export function createApiClient(token, fetchImpl = globalThis.fetch) {
  ...
  return Object.freeze({
    ...
    createAccount: (provider, label) => request("POST", "/api/accounts", { provider, label }),
    rename: (id, label) => request("PATCH", `/api/accounts/${fixedAccountId(id)}`, { label }),
    login: (id, provider) => request("POST", `/api/accounts/${fixedAccountId(id)}/login`, { provider }),
    refresh: (id, provider) => request("POST", `/api/accounts/${fixedAccountId(id)}/refresh`, { provider }),
    logout: (id, provider) => request("POST", `/api/accounts/${fixedAccountId(id)}/logout`, { provider }),
    remove: (id, provider, action) => request("DELETE", `/api/accounts/${fixedAccountId(id)}`, { provider, action }),
