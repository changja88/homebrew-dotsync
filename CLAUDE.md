...
- `lib/dotsync/diffinfo.py` — 프리뷰 요약(±줄 수, JSON/TOML 최상위 키, 바이너리 크기)과 d키 전체 diff 텍스트를 만드는 순수 함수 모듈. plan.py가 요약을, cli.py가 diff를 소비한다. 방향 규약은 항상 "dest가 source 내용으로 바뀐다".
