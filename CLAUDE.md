...
- `lib/dotsync/config.py` — `~/.config/dotsync/config.toml` 읽기/쓰기 (stdlib `tomllib`). `SUPPORTED_APPS`와 `Config` dataclass(BTT preset 포함)를 정의.
...
- `lib/dotsync/apps/{claude,ghostty,bettertouchtool,zsh}.py` — 구체 앱 모듈.
- `lib/dotsync/apps/__init__.py` — `APP_NAMES` + `build_app(name, cfg)` factory. config 의존(BTT preset)을 위해 정적 REGISTRY 대신 factory 사용.
