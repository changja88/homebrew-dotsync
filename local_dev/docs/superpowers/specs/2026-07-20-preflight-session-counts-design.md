...
2. Use `sessions`, `cleanup`, and `retention` rows. This keeps each value
   scannable and fits the existing box. This is the selected layout.
...
· cleanup     35 groups / 358 records to delete . 23 groups / 497 records to keep
· retention   delete when inactive longer than 5d . all known homes
```
...
· cleanup     75 records native cleanup . 33 records to keep
· retention   delete when inactive longer than 5d . all projects
```

When inventory scanning fails, the existing fail-closed behavior remains. The
`sessions`, `cleanup`, and `retention` rows show that the scan is unavailable
...
The retention text says `inactive longer than 5d`, matching the strict cutoff:
a session exactly five days old is kept, while a session older than five full
24-hour periods is eligible. Codex applies the cutoff to the newest record in a
logical group. Claude receives `cleanupPeriodDays: 5` and performs native
cleanup.
...
- Codex preflight renders all three rows and the explicit five-day policy;
