...
def _read_choice_arrow(
    ...
    *,
    ...
    """Render a small huh-style single-choice picker on a real TTY."""
    cursor = 0
    block_height = len(options) + 1

    def render(initial: bool) -> None:
        if not initial:
            stream.write(f"\x1b[{block_height}A\x1b[J")
        stream.write(f"  \x1b[{PURPLE}m?\x1b[0m {question}\n")
        for index, label in enumerate(options):
            if index == cursor:
                stream.write(
                    f"    \x1b[{PURPLE}m▶\x1b[0m "
                    f"\x1b[{PURPLE}m{label}\x1b[0m\n"
                )
            else:
                stream.write(f"      \x1b[90m{label}\x1b[0m\n")
        stream.flush()
    ...
    stream.write(f"\x1b[{block_height}A\x1b[J")
    if cancelled:
        stream.write(f"  \x1b[90m-\x1b[0m {question} cancelled\n")
        stream.flush()
        return None
    chosen = options[cursor]
    stream.write(
        f"  \x1b[{PURPLE}m?\x1b[0m {question} "
        f"\x1b[{PURPLE}m{chosen}\x1b[0m\n"
    )
    stream.flush()
    return chosen
...
def select_one(
    ...
    *,
    ...
    """Select one option, using arrows on a TTY and numbered input otherwise."""
    if not options:
        return None
    ...
    if input_fn is None and _RAW_TTY_AVAILABLE:
        ...
        if fd >= 0 and os.isatty(fd):
            return _read_choice_arrow(question, options, stream=out, fd=fd)

    for index, option in enumerate(options, start=1):
        out.write(f"  {index}) {option}\n")
    out.write(f"  > {question} [1-{len(options)}, q cancel] ")
    out.flush()
    reader = input_fn if input_fn is not None else input
    reply = reader().strip()
    if not reply:
        return options[0]
    if reply.lower() in {"q", "quit", "cancel"}:
        return None
    if reply in options:
        return reply
    try:
