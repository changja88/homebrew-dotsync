...
def test_render_zsh_shim_clears_codex_preflight_before_launch():
    ...
    assert "printf '\\e[3J\\e[H\\e[2J'" in codex_body
    assert codex_body.index("printf '\\e[3J\\e[H\\e[2J'") < codex_body.index('"$SERENA_AGENT_PYTHON" "$SERENA_AGENT_LAUNCHER" "$@"')
...
def test_render_zsh_shim_clears_claude_preflight_before_launch():
    ...
    assert "printf '\\e[3J\\e[H\\e[2J'" in claude_body
    assert claude_body.index("printf '\\e[3J\\e[H\\e[2J'") < claude_body.index('"$SERENA_AGENT_PYTHON" "$SERENA_AGENT_LAUNCHER" "$@"')
